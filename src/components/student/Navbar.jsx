    ...
    <div className={`flex items-center justify-between px-4 sm:px-10 md:px-14 lg:px-36
     border-b border-gray-500 py-4
      ${isCourseListPage ? 'bg-white' : 'bg-cyan-100/70'}`}>
        <img onClick={()=> navigate(`${baseUrl}`)} src={assets.learnify_2} alt="Logo" className='w-28 lg:w-44 cursor-pointer'/>
        <div className='hidden md:flex items-center gap-5 text-gray-500'>
            <div className='flex items-center gap-5'>
                ...
                { user ? <UserButton/> : 
                    <button onClick={()=> openSignIn()} className='cursor-pointer bg-blue-600 text-white px-5 py-2'>Create Account</button>}
            </div>
